...
TEST_F(MessageCacheTest, message_cache_handles_null_message) {
...
TEST_F(MessageCacheTest, message_cache_buffer_time_only_limits_by_duration) {
  ...
  // Expected 5 messages in buffer (2 seconds / 500ms + 1)
  ...
  // Push expected_msgs messages (should all be accepted)
  ...
  // Push remaining messages (will be dropped due to time limit)
  const size_t drop_msgs_start_index = expected_msgs;
  ...
  // Verify all messages are within the time window
