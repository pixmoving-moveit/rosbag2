...
TEST_F(CircularMessageCacheTest, circular_message_cache_handles_null_message) {
  auto circular_message_cache = std::make_shared<rosbag2_cpp::cache::CircularMessageCache>(500);

  // Should not throw when pushing null message
  ASSERT_NO_THROW(circular_message_cache->push(nullptr));

  // Test message with null serialized data
  auto msg = make_test_msg();
  msg->serialized_data = nullptr;
  ASSERT_NO_THROW(circular_message_cache->push(msg));
}
...
TEST_F(CircularMessageCacheTest, time_only_buffer_drops_old_messages_by_duration) {
  ...
  const auto oldest = messages.front()->time_stamp;
  const auto newest = messages.back()->time_stamp;
...
TEST_F(CircularMessageCacheTest, rejects_message_exceeding_size_limit) {
  ...
  const auto large_msg = make_test_msg(
    static_cast<rcutils_time_point_value_t>(RCUTILS_S_TO_NS(1)), std::string(100, 'x'));
  // Push single message larger than buffer - it will be dropped
  circular_message_cache->push(large_msg);

  circular_message_cache->notify_data_ready();
  circular_message_cache->swap_buffers();

  auto buffer = circular_message_cache->get_consumer_buffer()->data();
  // Buffer should be empty since the large message was dropped
...
TEST_F(CircularMessageCacheTest, handles_out_of_order_timestamps_gracefully) {
  ...
  const auto newer_message = make_test_msg(
    static_cast<rcutils_time_point_value_t>(
      std::chrono::duration_cast<std::chrono::nanoseconds>(newer_ts).count()));
  const auto older_message = make_test_msg(
    static_cast<rcutils_time_point_value_t>(
      std::chrono::duration_cast<std::chrono::nanoseconds>(base).count()));

  // Push newer message first
  circular_message_cache->push(newer_message);
  // Push older (out-of-order) message - it will be dropped
  ...
  auto buffer = circular_message_cache->get_consumer_buffer()->data();
  // Only the newer message should be present (older was dropped)
